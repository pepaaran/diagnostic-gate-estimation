...
# linear in the exposure mappings (categorical)
exposure_response = function(w, x, param){
  n = length(w)
  

  param$beta0[1] * (w == 0) + param$beta1[1] * (w == 1) + # direct effect
    param$beta0[2] * (x == "00") + param$beta0[3] * (x == "01") +
    param$beta1[2] * (x == "10") + param$beta1[3] * (x == "11") +
    rnorm(n, sd=param$noise_sd) # error term
}

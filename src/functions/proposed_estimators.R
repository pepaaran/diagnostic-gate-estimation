...
exposure_adjustment = function(data, vars = NULL){
  if (is.null(vars)) vars = names(data$x_obs)
  w = data$w
  y0 = data$y[w==0]
  y1 = data$y[w==1]
  ...
  # if( length(unique(x0$exp_map)) > 1){
  #   beta0 = lm(y0 ~ ., data = x0)$coef[1]
  # }else{
  #   beta0 <- mean(y0)
  # } 
  ...
  if(length(unique(x0$exp_map)) > 1){
    beta0 <- lm(y0 ~ ., data=x0)$coef[1]
  }else if("00" %in% unique(x1$exp_map)){
    beta0 <- mean(y0)
  } else {
    return("No observed exposure 00")
  }
  
  # if(length(unique(x0$exp_map))){
  #   beta1 <- lm(y1 ~ ., data = x1)$coef[2]
  # } else{
  #   beta1 <- mean(y1)
  # }
  
  if( length(unique(x1$exp_map)) > 1){
    beta1 = lm(y1 ~ ., data=x1)$coef[2]
  }else if("11" %in% unique(x1$exp_map)){
    beta1 <- mean(y1)
  }else{
    return("No observed exposure 11")
  }
    

  beta0 - beta1 # Not sure if this is the best way to go... because 11 and 00 may not be the 
  # exposures ever, and then what are we estimating?
}

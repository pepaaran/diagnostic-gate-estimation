# Simulation script for the first model variable and prob = 0.5 (without exposure mappings), passing parameters as a config file
...
output_file <- paste0("sim1linear05", network, network_size, ".csv")
...
params = purrr::cross(list(
  beta0 = list(c(0, 0, 0), c(0, 0.2, 0), c(0, 0.5, 0)),
  beta1 = list(c(1, 0, 0), c(1, 0.2, 0), c(1, 0.5, 0)),
...
run_sim = function(param, covariate_fns_for_response, vars, prob, nperm, generate_network, network_size, pid = NULL) {
  ...
  # Geary's test
  geary0 = geary.mc(res0, listw = glistw,
                    nsim = nperm)$p.value < 0.05
...
########Check memory requirements of simulation#########
# data = generate_covariate_data(g, covariate_fns_for_response, prob) # also generates treatment assignment
# # generate response
# data$y = linear_response(data$w, data$x_obs, list(beta0 = c(0, 0.5, 0, 0), beta1 = c(1, 0.5, 0, 0), noise_sd = 0.1))
# # Baseline model residuals
# res0 <- data$y - mean(data$y[data$w==0])
# res0[data$w==1] <- data$y[data$w==1] - mean(data$y[data$w==1])
# res0 <- as.vector(res0)
...
# profvis::profvis(
#   for(k in 1:100){
#     cond_perm_test(res0, w = data$w, g = g, focal = F_epsnet, covariate_fn = fraction_trt_nbrs, method = "spearman", alternative = "greater", nperm = 100, conf.level = 0.05)
#   }
# )
# 
# profvis::profvis(
#   for(k in 1:100){
#     cond_perm_test(res0, w = data$w, g = g, focal = F_epsnet, covariate_fn = fraction_trt_nbrs, method = "spearman", alternative = "greater", nperm = 100)$p.value < 0.05
#   }
# )

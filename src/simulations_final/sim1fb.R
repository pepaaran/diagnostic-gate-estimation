# Simulation script for the first model variable and facebook network, passing parameters as a config file
...
nsim_save =20 # (with 100 it runs out of memory with mem=20000 per core)
nperm = 500 # number of permutations for permutation tests
# Here the probability must be passed as a parameter

# Define changing parameters: network, prob, response
source(as.character(commandArgs(trailingOnly=TRUE)))

output_file <- paste0("sim1",response, "_", network, "_pi", prob, ".csv")
...
if(response == "linear") params = purrr::cross(list(
  beta0 = list(c(0, 0, 0, 0), c(0, 0.2, 0, 0), c(0, 0.5, 0, 0)),
  beta1 = list(c(1, 0, 0, 0), c(1, 0.2, 0, 0), c(1, 0.5, 0, 0)),
...
run_sim = function(param, g, covariate_fns_for_response, vars, vf, Nf, prob, nperm, response, pid = NULL) {
                 ...
                 function(x){
                   ...
                   c(geary = geary.mc(res, listw = glistw, nsim =nperm)$p.value < 0.05,
                     ...
                     athey_compare_epsnet1 = cond_perm_test(res, w = data$w, g = g, focal = F_epsnet, 
                                                            ...
                                                            method = "spearman", alternative = "greater", nperm = nperm, conf.level = 0.05),
                     athey_compare_epsnet2 = cond_perm_test(res, w = data$w, g = g, focal = F_epsnet, 
                                                            covariate_fn = covariate_fns_for_response[[other[[2]] ]],
                                                            method = "spearman", alternative = "greater", nperm = nperm, conf.level = 0.05),
                     athey_compare_random1 = cond_perm_test(res, w = data$w, g = g, focal = F_random, 
                                                            covariate_fn = covariate_fns_for_response[[other[[1]] ]],
                                                            method = "spearman", alternative = "greater", nperm = nperm, conf.level = 0.05),
                     athey_compare_random2 = cond_perm_test(res, w = data$w, g = g, focal = F_random, 
                                                            covariate_fn = covariate_fns_for_response[[other[[2]] ]],
                                                            method = "spearman", alternative = "greater", nperm = nperm, conf.level = 0.05))
                 })
  ...
  return(data.frame(
    ...
    adj3=data %>% linear_adjustment(vars=vars$vars3),
    ...
    var3 = if(is.null(vf$vars3)) NA else data %>% linear_variance_estimate(vf$vars3, vars=vars$vars3),
    ...
    athey3_epsnet = tests[2,3],
    
    athey1_random = tests[3,1],
    athey2_random = tests[3,2],
    athey3_random = tests[3,3],
    
    athey12_epsnet = tests[4,1],
    athey13_epsnet = tests[5,1],
    athey21_epsnet = tests[4,2],
    athey23_epsnet = tests[5,2],
    athey31_epsnet = tests[4,3],
    athey32_epsnet = tests[5,3],
    
    athey12_random = tests[6,1],
    athey13_random = tests[7,1],
    athey21_random = tests[6,2],
    athey23_random = tests[7,2],
    athey31_random = tests[6,3],
    athey32_random = tests[7,3]
...
run_sim(params[[1]], g, covariate_fns_for_response, vars, vf, Nf, prob, nperm, response, pid = 1)
        ...
        .packages = c('dplyr', 'broom', 'spdep', 'ade4', 'igraph')) %do% {
          ...
          estimates = foreach(rep = 1:nsim_save, .combine=rbind,
                              ...
                              .packages = c('dplyr', 'broom', 'spdep', 'ade4', 'igraph')) %dopar% {
                                run_sim(param, g, covariate_fns_for_response, vars, vf, Nf, prob, nperm, response, pid = i)
                              } %>% data.frame
